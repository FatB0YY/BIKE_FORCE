...
import { useCreateBrandMutation } from '../../../service/BrandsAPI'
...
const BrandsForm = () => {
  // rtk query
  const [createBrand, { isLoading: isLoadingCreateBrand }] = useCreateBrandMutation()
  // react hook form
  const {
    register,
    handleSubmit,
  ...
  const onValidSubmit: SubmitHandler<MyForm> = async ({ name }) => {
    await createBrand(name.toLocaleLowerCase())
      .unwrap()
      .catch((error) => {
        console.error(error)
        // Обработка ошибки
  ...
  return (
    <form onSubmit={handleSubmit(onValidSubmit)}>
      <div>
        <h2 className='text-center text-2xl mb-3 text-gray-900'>Create Brand</h2>
      ...
      <button
        disabled={isLoadingForm || isLoadingCreateBrand}
        type='submit'
      ...
      >
        {isLoadingForm || isLoadingCreateBrand ? (
          <SpinnerButton
