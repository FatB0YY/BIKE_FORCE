...
import { useGetRoleByNameQuery } from '../service/RolesAPI'
...
const RegistrationForm = () => {
  ...
  const [registration, { isLoading: isLoadingRegistration, isSuccess }] = useRegistrationMutation()
  const { isLoading: isLoadingGetRoleByNameQuery, data: role = null } = useGetRoleByNameQuery('USER')
  ...
  const onValidSubmit: SubmitHandler<MyForm> = async ({ email, password }) => {
    ...
    if (role) {
      const roles = [role]
      await registration({ email, password, roles })
        .unwrap()
        .then(() => {
  ...
  const isValidEmail = (data: string) => {
    ...
    const maxLength = 200
    return emailRegex.test(data) && data.length <= maxLength
  }
  ...
  // переделать, /products доступен только админам (hoc компонент готов)
  useEffect(() => {
    if (user) navigate('/products')
  }, [navigate, user])

  if (isLoadingGetRoleByNameQuery) {
